...
class App:
    ...
    def show_result(self) -> None:
        '''результат викторины'''
        tkinter.Label(
            self.main_frame, text='Викторина завершена!'
            ).pack()
        tkinter.Label(
            self.main_frame, text=f'Всего вопросов: {len(questions)}'
            ).pack()
        tkinter.Label(
            self.main_frame, text=f'Правильных ответов: {self.right_anwers}'
            ).pack()
        tkinter.Label(
            self.main_frame, text=f'Ошибок: {self.wrong_anwers}'
            ).pack()
        tkinter.Button(
            self.main_frame, text='начать заного', command=self.start
            ).pack()
